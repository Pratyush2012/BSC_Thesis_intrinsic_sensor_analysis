...
\begin{itemize}
    ...
    \item \textbf{Domain transfer evaluation dropped.} The campus and farm label spaces only partially overlap, so the two datasets are reported as independent classification problems rather than via direct train-on-campus / test-on-farm transfer.
    \item \textbf{Additional analyses added (18 April – 9 May):} sensor-zeroing and baseline transition detection as new evaluation axes; a three-class matched campus subset for a like-for-like comparison with the farm; and a CUSUM change detector on classifier posteriors as a principled alternative transition detector, following \cite[\S6.4]{Blanke2003}.
...
\Cref{ch:background} reviews the relevant background on proprioceptive terrain classification, IMU signals, and the feature families and supervised learning methods used in this work. \Cref{ch:setup} and \Cref{ch:collection} then describe the Ricbot platform, its sensor suite, the two outdoor data-collection campaigns (campus development and main agricultural), and the manual video-aided labelling procedure.

The methodology is presented in three chapters: \Cref{ch:processing} covers the preprocessing pipeline (QC, synchronisation, cleaning, filtering, sliding-window segmentation); \Cref{ch:feature} the 189 time and frequency-domain features, the speed-regime A/B dataset split, and the mutual-information ranking with correlation pruning; and \Cref{ch:classification} the six classifier families and the Leave-One-Run-Out cross-validation protocol.

\Cref{ch:results} reports the full numerical findings: classification performance on both datasets, sensor-ablation, and transition detection, and \Cref{ch:discussion,ch:conclusion} interpret them and outline future work.

In brief, the thesis shows that intrinsic motion signals alone are sufficient to recognise terrain on both scenarios, that careful feature engineering keeps classical learners competitive with a 1D-CNN, and that the accelerometer and gyroscope carry the bulk of the discriminative information.